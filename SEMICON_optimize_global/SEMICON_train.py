...
def train(query_dataloader, train_dataloader, retrieval_dataloader, code_length, args):
    ...
    criterion = ADSH_Loss(code_length, args.gamma, args.dataset, args.device)
    ...
    for it in range(args.max_iter):
        ...
        for epoch in range(args.max_epoch):
            ...
            for batch, (data, targets, index) in pbar:
                ...
                U[index, :] = F.data
                # print('-----')
                # print('F', F.shape)  # F 表示结果的 hash 编码, (batch_size, 48)
                # print('data', data.shape)
                # print('targets', targets.shape)  # 表示结果的label, (batch_size, 200)
                # print('U', U.shape)
                # print('index', index)
                # print(targets)
                cnn_loss, hash_loss, quan_loss = criterion(F, B, S[index, :], sample_index[index], targets)
